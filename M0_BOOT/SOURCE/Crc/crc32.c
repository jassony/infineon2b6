...
uint32 calculate_crc32(uint32 *data, uint32 len)
{
...
{
//    uint08 data[8] = {1, 2, 3, 4, 5, 6, 7, 8};
//    uint32 crc32 = calculate_crc32(data, 8);
}
#endif
 ...
 */
 /*************************************************************************************************/
uint32 CalcCrc32(uint32 crcInit, uint08* pBuf, uint32 len)
{
    uint32 crc = crcInit;
...
//    create_crc32_table();
    ...
    while(len > 0)
    {
        crc = crc32Table[*pBuf ^ (uint08)crc] ^ (crc >> 8);
        pBuf++;
        len--;
    }
...
/* 一段数据分多次校验时，应设置初始值为0，随后作为参数传递进去的初始值都需要异或0xffffffff */
uint32 init_crc = 0;
...
void crc32_test(void)
{
    ...
    init_crc = CalcCrc32(init_crc, (uint08*)&data[0], 4);
    init_crc = CalcCrc32(init_crc, (uint08*)&data[4], 4);
}
