function [pos_pu, speed_pu, lambda_alpha1_out, lambda_beta1_out, lambda_alpha2_out, lambda_beta2_out, dhat_alpha_out, dhat_beta_out, omega_fast_out, omega_slow_out, kdf_mode, kaf_mode, flux_limited] = fado_estimator_step(VI_fb, enable, fado_cfg)
%FADO_ESTIMATOR_STEP One discrete SI-unit step of the FADO estimator.
% Implements the observer and limiter described by equations (9), (10),
% (16), (20), (21), and (24) of the supplied paper.

% Fixed ordering keeps the MATLAB Function interface code-generation safe.
% [Rs RsScale Lq LqScale p Ts Vbase Ibase Nbase fluxLimit Kdf lowHz Kaf
%  fastHz slowHz zeta voltagePreprocess vAlphaOffset vBetaOffset]
rs_nom = fado_cfg(1);
rs_scale = fado_cfg(2);
lq_nom = fado_cfg(3);
lq_scale = fado_cfg(4);
pole_pairs = fado_cfg(5);
Ts_fado = fado_cfg(6);
v_base = fado_cfg(7);
i_base = fado_cfg(8);
n_base = fado_cfg(9);
flux_limit = fado_cfg(10);
kdf = fado_cfg(11);
low_speed_hz = fado_cfg(12);
kaf = fado_cfg(13);
fast_bandwidth_hz = fado_cfg(14);
slow_bandwidth_hz = fado_cfg(15);
zeta = fado_cfg(16);
enable_voltage_preprocess = fado_cfg(17) ~= 0;
v_alpha_preprocess = fado_cfg(18);
v_beta_preprocess = fado_cfg(19);
...
% Convert existing alpha-beta per-unit feedback before the flux integrations.
v_alpha = VI_fb(1)*v_base;
v_beta = VI_fb(2)*v_base;
i_alpha = VI_fb(3)*i_base;
i_beta = VI_fb(4)*i_base;

% Reserved hook for measured-voltage and inverter nonlinearity compensation.
...
omega_threshold = 2*pi*low_speed_hz;
kaf_mode = abs(omega_fast) <= omega_threshold;
kdf_mode = ~kaf_mode;
flux_limited = false;

% Equation (21): normal-speed disturbance feedback. Equation (24): limiter.
...
% Equations (9), (10), and (16): adaptive Luenberger observer.
...
lambda_alpha_hat_dot = -omega_fast*lambda_beta_hat + omega_fast*dhat_beta + p1*innovation_alpha + p3*innovation_beta;
lambda_beta_hat_dot = omega_fast*lambda_alpha_hat - omega_fast*dhat_alpha + p2*innovation_alpha + p4*innovation_beta;
dhat_alpha_dot = q1*innovation_alpha + q3*innovation_beta;
dhat_beta_dot = q2*innovation_alpha + q4*innovation_beta;
...
% Equation (20) and direct electrical rotor-angle estimation.
...
% The fast T2S drives the adaptive frequency and FOC electrical position.
wn_fast = 2*pi*fast_bandwidth_hz;
phase_error_fast = fado_wrap_to_pi(theta_direct - theta_fast);
omega_fast = omega_fast + Ts_fado*wn_fast*wn_fast*phase_error_fast;
theta_fast = fado_wrap_to_pi(theta_fast + Ts_fado*(omega_fast + 2*zeta*wn_fast*phase_error_fast));

% The slow T2S supplies damped mechanical per-unit speed to the speed loop.
wn_slow = 2*pi*slow_bandwidth_hz;
phase_error_slow = fado_wrap_to_pi(theta_direct - theta_slow);
omega_slow = omega_slow + Ts_fado*wn_slow*wn_slow*phase_error_slow;
theta_slow = fado_wrap_to_pi(theta_slow + Ts_fado*(omega_slow + 2*zeta*wn_slow*phase_error_slow));
...
function angle_out = fado_wrap_to_pi(angle_in)
