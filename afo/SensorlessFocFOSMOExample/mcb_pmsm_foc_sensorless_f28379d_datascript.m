...
ObsType = get_param([bdroot '/Current Control/Input Scaling/Calculate position and speed/EstimatorSelector'],'Value');

FOCutOffFrq = MAX_OL_POS_SPD*(PWM_frequency/15e3)*(PU_System.N_base/60)/10;
FO.Type = 'FO';
FO.Parameters.CutOffFrq = FOCutOffFrq;
FOSpdCutOffFrq = (PU_System.N_base/60)/10;
FOSpdFltCoeff = (2*pi*FOSpdCutOffFrq*Ts)/(1+(2*pi*FOSpdCutOffFrq*Ts));
FOSLDelay.Gain = 1;
...
switch ObsType
    case '0' % Sliding Mode Observer
        PI_params = mcb.getPIControllerParameters(pmsm,inverter,PU_System,T_pwm,2*Ts,2*Ts_speed);

    case '1' % Flux Observer
        
        PI_params = mcb.calcFOCGains(motor,Ts,Ts_speed,Base=PU_System,PositionObserver=FO,SLFeedbackPathDelay=FOSLDelay);
	    % In case of sensroless, changing symmetry factor value of Symmetric Optimum from 2 to 1.2
	    PI_params.Kp_speed = PI_params.Kp_speed*2/1.2;
	    PI_params.Ki_speed= PI_params.Ki_speed*8/(1.2)^3;

    case '2' % Extended EMF Observer
        
        PI_params = mcb.calcFOCGains(motor,Ts,Ts_speed,Base=PU_System,PositionObserver=EEMF,SLFeedbackPathDelay=EEMFSLDelay);
        % In case of sensroless, changing symmetry factor value of Symmetric Optimum from 2 to 1.2
	    PI_params.Kp_speed = PI_params.Kp_speed*2/1.2;
	    PI_params.Ki_speed= PI_params.Ki_speed*8/(1.2)^3;
