...
MTABYTEPTR ApplXcpGetPointer( vuint8 addr_ext, vuint32 addr )
{
  ...
  return (MTABYTEPTR)(uintptr_t)addr;
}
...
******************************************************************************/
  ...
  #else
static vuint8 XcpApplIsWriteRangeAllowed(const uintptr_t requestStart,
                                         const uintptr_t requestEnd,
                                         const uintptr_t rangeStart,
                                         const uintptr_t rangeSize)
{
  const uintptr_t rangeEnd = rangeStart + rangeSize;

  if ((requestEnd < requestStart) || (rangeEnd < rangeStart))
  {
...
{
  const uintptr_t requestStart = (uintptr_t)address;
  const uintptr_t requestEnd = requestStart + (uintptr_t)size;
  const uintptr_t xcpStart = (uintptr_t)&xcp;
  const uintptr_t xcpEnd = xcpStart + (uintptr_t)sizeof(xcp);

  if ((size == 0u) || (requestEnd < requestStart) || (xcpEnd < xcpStart))
  {
    return (vuint8)XCP_CMD_DENIED;
  }

  /* Protect protocol state and DAQ bookkeeping from DOWNLOAD. */
  if ((requestStart < xcpEnd) && (requestEnd > xcpStart))
  {
    return (vuint8)XCP_CMD_DENIED;
  }

  /*
   * CM0+ hosts XCP, while M0/M4 application globals occupy separate areas of
   * the shared CYT2B6 SRAM. Let CANape calibrate every stable M0/M4 variable
   * in those regions. This also covers future globals without changing the
   * XCP source or A2L; the caller only needs its linked ECU address.
   */
  if ((XcpApplIsWriteRangeAllowed(requestStart, requestEnd,
                                  XCP_M0_STATIC_RAM_ADDRESS,
                                  XCP_M0_STATIC_RAM_SIZE) == (vuint8)XCP_CMD_OK) ||
      (XcpApplIsWriteRangeAllowed(requestStart, requestEnd,
                                  XCP_M4_STATIC_RAM_ADDRESS,
                                  XCP_M4_STATIC_RAM_SIZE) == (vuint8)XCP_CMD_OK))
  {
    return (vuint8)XCP_CMD_OK;
  }
