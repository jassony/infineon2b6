...
/* Constants                                                                */
/****************************************************************************/
XcpCanMessageType Xcp_message_RXD;
XcpCanMessageType Xcp_message_TXD;
...
const vuint8 PTS_Passwords[7] = {'D','E','S','A','Y','S','V'};
vuint8 PTSActive;
vuint8 XcpPreCopy( PRECOPY_PARAM_TYPE PRECOPY_PARAM )
{
  ...
  #if defined ( V_ENABLE_USE_DUV_ENABLE_USE_DUMMY_STATEMENTMMY_STATEMENT )
      ...
      */
      ...
      if(PTSActive == 0u)
      {
          if(c.b[0]==0x27u)
          {
            ...
            for(u8i=0;u8i<7;u8i++)
            {
                if(PTS_Passwords[u8i]!=c.b[u8i+1])
                {
                    PTSActive = 0u;
                }
            }
      ...
      if(PTSActive == 1u)
      {
           if((c.b[0]==0x10u)&&(c.b[1]==0x20u))
           {
                /*Exit PTS mode*/
                PTSActive = 0;
           }
          return( (vuint8)kCanCopyData );
      }
