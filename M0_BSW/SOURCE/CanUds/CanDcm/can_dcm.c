...
//    if (s_dcm_start_cnt < DCM_START_TIME)
//    {
//        s_dcm_start_cnt++;
//        s_s3server_cnt = 0;
//        return;
...
void dcm_1001_nrc78_form_boot_set(PDU_ID hrh)
{    
    s_dcm_rx_flg[hrh] = 1;
    s_dcm_req_len[hrh] = 2;
    s_dcm_rx_buf[hrh][0] = 0x10;
    s_dcm_rx_buf[hrh][1] = 0x01;
}
...
void app_dtc_setting(stDCM_MSG_CONTEXT* msg, stDCM_SERVICE_CFG service_cfg)
{
    ...
    {
        msg->nrc = NRC_SERVICE_NOT_SUPPORTED_IN_ACTIVE_SESSION;
        dcm_service_85_dtcsettingtype_set(0);
    }
    else if (msg->req_data_len < valid_min_len)
    {
        msg->nrc = NRC_INCORRECT_MSG_LENGTH_OR_INVALID_FORMAT;
...
void app_read_dtc_information(stDCM_MSG_CONTEXT* msg, stDCM_SERVICE_CFG service_cfg)
{
    ...
    if (NRC_POSITIVE_RESPONSE == msg->nrc)
    {
        ...
        if (DCM_SID19_SUB_01 == sub_id)
        {
            num = dtc_number_get_by_sts(msg->req_data[2]);
            msg->res_data_len = 6;
            msg->res_data[1] = sub_id;
            msg->res_data[2] = DTC_SUPPORTED_STS;
            msg->res_data[3] = DTC_FORMAT_14229_1;
            msg->res_data[4] = ((num >> 8U) & 0xFF);
            msg->res_data[5] = ((num >> 0U) & 0xFF);
        }
        else if (DCM_SID19_SUB_02 == sub_id)
        {
            msg->res_data[1] = sub_id;
            msg->res_data[2] = DTC_SUPPORTED_STS;   
            msg->res_data_len = 3;
            ...
            {
                if (msg->req_data[2] & dtc_sts_get(typ))
                {
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 16U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 8U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 0U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = dtc_sts_get(typ);
                    msg->res_data_len++;
                }
            }
        }
        else if (DCM_SID19_SUB_03 == sub_id)
        {
            msg->res_data[1] = sub_id;
            msg->res_data_len = 2;  
            for (typ = (eDTC_TYP)0; typ < CAN_DTC_NUM; typ++)
            {
                if (DEF_TRUE == is_dtc_snapshot_recorded(typ, DTC_SNAPSHOT_GLOBAL_RECORD_NUMBER))
                {
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 16U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 8U) & 0xFF);
                    ...
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 0U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = DTC_SNAPSHOT_GLOBAL_RECORD_NUMBER;
                    msg->res_data_len++;
                }
                else if (DEF_TRUE == is_dtc_snapshot_recorded(typ, DTC_SNAPSHOT_LOCAL_RECORD_NUMBER))
                {
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 16U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 8U) & 0xFF);
                    ...
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 0U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = DTC_SNAPSHOT_LOCAL_RECORD_NUMBER;
                    msg->res_data_len++;
                }
                else if (DEF_TRUE == is_dtc_snapshot_recorded(typ, DTC_SNAPSHOT_ALL_RECORD_NUMBER))
                {
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 16U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 8U) & 0xFF);
        ...
        else if (DCM_SID19_SUB_04 == sub_id)
        {
            if (   (msg->req_data[5] != DTC_SNAPSHOT_GLOBAL_RECORD_NUMBER)
                && (msg->req_data[5] != DTC_SNAPSHOT_LOCAL_RECORD_NUMBER)
                && (msg->req_data[5] != DTC_SNAPSHOT_ALL_RECORD_NUMBER)
                )
            {
                msg->nrc = NRC_REQUEST_OUT_OF_RANGE;
            }
            ...
            {
                req_dtc = (uint32)((uint32)msg->req_data[2] << 16U) + 
                          (uint32)((uint32)msg->req_data[3] << 8U) +
                          (uint32)((uint32)msg->req_data[4] << 0U);
                for (typ = (eDTC_TYP)0; typ < CAN_DTC_NUM; typ++)
                {
                    if (req_dtc == dtc_cfg[typ].dtc)
                    {
                        break;
                    }
                    else {}
                ...
                else
                {
                    ...
                    msg->res_data[msg->res_data_len] = dtc_sts_get(typ);
                    msg->res_data_len++;
                    if (DEF_TRUE == is_dtc_snapshot_recorded(typ, msg->req_data[5]))
                    {
                        
                        if (DTC_SNAPSHOT_GLOBAL_RECORD_NUMBER == msg->req_data[5]||DTC_SNAPSHOT_ALL_RECORD_NUMBER == msg->req_data[5])
                        {
                            msg->res_data[msg->res_data_len] = DTC_SNAPSHOT_GLOBAL_RECORD_NUMBER;
                            msg->res_data_len++;
                            dtc_shapshot_global_data_get(typ, (stDTC_SNAPSHOT*)&snapshot);
                            msg->res_data[msg->res_data_len] = DTC_SNAPSHOT_GLOBAL_DID_NUMBER;
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N1 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N1 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.runCmd >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N2 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N2 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.spdCmd >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.spdCmd >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N3 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N3 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.Max_power_acc >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.Max_power_acc >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N4 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N4 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.BusCurrent >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N5 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N5 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.PhaseCurrent >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N6 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N6 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.Temp_COM_SIC >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N7 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N7 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.speed_actal >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.speed_actal >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N8 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N8 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.fault_status_flag_motor >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.fault_status_flag_motor >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N9 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N9 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.run_time >> 24U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.run_time >> 16U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.run_time >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.run_time >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N10 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N10 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.pVdcValue >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.pVdcValue >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N11 >> 8U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N11 >> 0U) & 0xFF);
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((snapshot.Temp_PCB >> 0U) & 0xFF);
                            msg->res_data_len++;

                        }
//                        if (DTC_SNAPSHOT_ALL_RECORD_NUMBER == msg->req_data[5])
//                        {
//                            msg->res_data[msg->res_data_len] = DTC_SNAPSHOT_GLOBAL_RECORD_NUMBER;
//                            msg->res_data_len++;
//                            dtc_shapshot_global_data_get(typ, (stDTC_SNAPSHOT*)&snapshot);
...
//                            msg->res_data_len++;
//                        }
                        if (DTC_SNAPSHOT_LOCAL_RECORD_NUMBER == msg->req_data[5]||DTC_SNAPSHOT_ALL_RECORD_NUMBER == msg->req_data[5])
                        {
                            msg->res_data[msg->res_data_len] = DTC_SNAPSHOT_LOCAL_RECORD_NUMBER;
                            msg->res_data_len++;
                            dtc_shapshot_local_data_get(typ, (stDTC_SNAPSHOT*)&snapshot);
                            msg->res_data[msg->res_data_len] = DTC_SNAPSHOT_GLOBAL_DID_NUMBER;
                            msg->res_data_len++;
                            msg->res_data[msg->res_data_len] = ((DTC_SNAPSHOT_GLOBAL_DID_N1 >> 8U) & 0xFF);
                            msg->res_data_len++;
        ...
        else if (DCM_SID19_SUB_06 == sub_id)
        {
            req_dtc = (uint32)((uint32)msg->req_data[2] << 16U) + 
                          (uint32)((uint32)msg->req_data[3] << 8U) +
                          (uint32)((uint32)msg->req_data[4] << 0U);
            ...
            else
            {
                ...
                msg->res_data_len = 2;
                num = 2;
                msg->res_data[num] = ((dtc_cfg[typ].dtc >> 16U) & 0xFF);
                num++;
                msg->res_data_len++;
                msg->res_data[num] = ((dtc_cfg[typ].dtc >> 8U) & 0xFF);
                num++;
                msg->res_data_len++;
                msg->res_data[num] = ((dtc_cfg[typ].dtc >> 0U) & 0xFF);
                num++;
                msg->res_data_len++;
                msg->res_data[num] = dtc_sts_get(typ);
                num++;
                msg->res_data_len++;
                #if 0 /* no extended data */
                msg->res_data[num] = msg->req_data[5];
                num++;
                msg->res_data_len++;
                #endif
            }
        }
        else if (DCM_SID19_SUB_0A == sub_id)
        {
            msg->res_data[1] = sub_id;
            msg->res_data_len = 2;
            msg->res_data[msg->res_data_len] = DTC_SUPPORTED_STS;
            msg->res_data_len++;
            for (typ = (eDTC_TYP)0; typ < CAN_DTC_NUM; typ++)
            {
//                if (DTC_SUPPORTED_STS & dtc_sts_get(typ))
                {
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 16U) & 0xFF);
                    msg->res_data_len++;
                    msg->res_data[msg->res_data_len] = ((dtc_cfg[typ].dtc >> 8U) & 0xFF);
...
{
    stDCM_DATA_PARAM_CFG* dp_cfg = service_cfg.dp;
    stDCM_IOCTRL_CFG* did_cfg = DEF_NULL;
    uint16 i = 0;
    uint16 j = 0;
    uint32 temp = 0;
    uint32 temp1 = 0;
    uint08 req_invalid = 0;
    uint08 valid_min_len = 0;
    uint08 valid_len = 0;
    uint16 req_did_num = 0;
    ...
    req_did_num = msg->req_data[1];
    req_did_num <<= 8U;
    req_did_num += msg->req_data[2];
    
    if (DEF_NULL != dp_cfg)
    {
        did_cfg = (stDCM_IOCTRL_CFG*)dp_cfg->data;
        for (i = 0; i < DCM_IODID_NUM; i++)
        {
            
            if (did_cfg[i].did == req_did_num)
            {
                req_invalid = 1;
                valid_len += did_cfg[i].cs_len;
                break;
            }
            else {}
        }
...
void app_routine_control(stDCM_MSG_CONTEXT* msg, stDCM_SERVICE_CFG service_cfg)
{
    ...
    else if ((DCM_SID_SUB_MAX == sub_id) || (DEF_NULL == subfunc_cfg))
    {
        msg->nrc = NRC_SUBFUNC_NOT_SUPPORTED;
            ...
            && ((service_cfg.security_lv & s_cur_security) != s_cur_security) 
            ) /* 33 for RID */
    {
        msg->nrc = NRC_SECURITY_ACCESS_DENIED;
    }
