...
static const uint16                 s_can0_tx_period_cfg[CAN0_TX_NUM] = 
{
  10
...
uint16 g_vehicle_speed;
uint16 g_vehicle_speed_original;
...
void Can_Transmit(cy_pstc_canfd_type_t chn, uint32_t canid, uint8_t dlc, uint8_t * data,bool fd)
{
    ...
    uint8_t i = 0;
    uint8_t tx_buffer_limit = TX_BUFFER_NUM;
    if( chn == CAN0_INSTANCE && 1 == s_can0_tx_disable) return;

    if(chn == CAN0_INSTANCE)
    {
      tx_buffer_limit = XCP_CAN_TX_BUFFER_INDEX;
    }
...
static void can_tx_process(void)
{
    /* 0x666 BusOff frame removed — no longer transmitted */
}
