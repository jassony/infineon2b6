...
static uint8_t work_flash_buf[2048];
...
//	status_t ret = STATUS_SUCCESS;
        ...
        if(eeprom_addr > CY_WFLASH_LG_SBM_BASE && eeprom_addr < CY_WFLASH_SM_SBM_BASE)
        {
          ...
          if(exceed + length > CY_WORK_LES_SIZE_IN_BYTE)
          {
            ...
            Work_Flash_Read(eepromaddress,work_flash_buf,CY_WORK_LES_SIZE_IN_BYTE);
//            common_memcpy((uint08*)work_flash_buf, (uint08*)eepromaddress, CY_WORK_LES_SIZE_IN_BYTE);
//            work_flash_buf[0] = *(uint32_t*)eepromaddress;
            common_memcpy((uint08*)(work_flash_buf + exceed),data_buf, eepromsize);
            Work_Flash_Erase(eepromaddress,CY_WORK_LES_SIZE_IN_BYTE);
            Work_Flash_Write(eepromaddress,work_flash_buf,CY_WORK_LES_SIZE_IN_BYTE);
            
            Work_Flash_Read(eepromaddress + CY_WORK_LES_SIZE_IN_BYTE,work_flash_buf,CY_WORK_LES_SIZE_IN_BYTE);
            common_memcpy((uint08*)work_flash_buf,data_buf + (CY_WORK_LES_SIZE_IN_BYTE - exceed), length - eepromsize);
            Work_Flash_Erase(eepromaddress + CY_WORK_LES_SIZE_IN_BYTE,CY_WORK_LES_SIZE_IN_BYTE);
            Work_Flash_Write(eepromaddress + CY_WORK_LES_SIZE_IN_BYTE,work_flash_buf,CY_WORK_LES_SIZE_IN_BYTE);
          }
          else
          {
            Work_Flash_Read(eepromaddress,work_flash_buf,CY_WORK_LES_SIZE_IN_BYTE);
//            common_memcpy((uint08*)work_flash_buf, (uint08*)eepromaddress, CY_WORK_LES_SIZE_IN_BYTE);
        ...
        else if(eeprom_addr >= CY_WFLASH_SM_SBM_BASE && eeprom_addr < (CY_WFLASH_SM_SBM_BASE + CY_WFLASH_SM_SBM_SIZE))
        {
          ...
          {
            Work_Flash_Read(eepromaddress,work_flash_buf,CY_WORK_SES_SIZE_IN_BYTE);
            Work_Flash_Read(eepromaddress+CY_WORK_SES_SIZE_IN_BYTE,work_flash_buf+CY_WORK_SES_SIZE_IN_BYTE,CY_WORK_SES_SIZE_IN_BYTE);
//            common_memcpy((uint08*)work_flash_buf, (uint08*)eepromaddress, CY_WORK_SES_SIZE_IN_BYTE*2);
            common_memcpy((uint08*)(work_flash_buf + exceed),data_buf, length);
            Work_Flash_Erase(eepromaddress,CY_WORK_SES_SIZE_IN_BYTE*2);
            Work_Flash_Write(eepromaddress,work_flash_buf,CY_WORK_SES_SIZE_IN_BYTE*2);
          }
          else
          {
            Work_Flash_Read(eepromaddress,work_flash_buf,CY_WORK_SES_SIZE_IN_BYTE);
//            common_memcpy((uint08*)work_flash_buf, (uint08*)eepromaddress, CY_WORK_SES_SIZE_IN_BYTE);
            ...
            Work_Flash_Write(eepromaddress,work_flash_buf,CY_WORK_SES_SIZE_IN_BYTE);
          }
...
//        ret = DEF_FALSE;
//    }
    Flash_Write_Sync(addr,data,len); /* blocking: bootloader must know the block is programmed */
...
//        ret = DEF_FALSE;
//    }
    Flash_Erase_Sync(addr,size); /* blocking: bootloader must know the region is erased */
