...
_st_adc_init adc_cfg[ADC_CN_NUM] = {
  ...
  
  {
    ...
    .sampleTime = 70, // 厚膜加热采样点 70*62.5 = 4375ns =4.375us
    .isGroupEnd = true,
    .grpDone = true,
    .ch = ADC1_HEAT_M
  },
...
static _st_adc_result s_adcChResult[ADC1_SOFT_NUM+ADC2_SOFT_NUM];

void Adc_Process(void)
{
  Cy_Adc_Channel_SoftwareTrigger(&PASS0_SAR1->CH[ADC1_HEAT_OUT]);
  Cy_Adc_Channel_SoftwareTrigger(&PASS0_SAR2->CH[ADC2_PCB_SENSE]);
  Cy_Adc_Channel_GetResult(&PASS0_SAR1->CH[ADC1_HEAT_OUT], &s_adcChResult[ADC_HEAT_OUT].value, &s_adcChResult[ADC_HEAT_OUT].adcChStatus);
  Cy_Adc_Channel_GetResult(&PASS0_SAR1->CH[ADC1_HEAT_IN], &s_adcChResult[ADC_HEAT_IN].value, &s_adcChResult[ADC_HEAT_IN].adcChStatus);
  Cy_Adc_Channel_GetResult(&PASS0_SAR1->CH[ADC1_T_SENSE_SIC_COMPRESS], &s_adcChResult[ADC_T_SENSE_SIC_COMPRESS].value, &s_adcChResult[ADC_T_SENSE_SIC_COMPRESS].adcChStatus);
  Cy_Adc_Channel_GetResult(&PASS0_SAR1->CH[ADC1_T_SENSE_SIC_HEAT], &s_adcChResult[ADC_T_SENSE_SIC_HEAT].value, &s_adcChResult[ADC_T_SENSE_SIC_HEAT].adcChStatus);
  Cy_Adc_Channel_GetResult(&PASS0_SAR2->CH[ADC2_PCB_SENSE], &s_adcChResult[ADC_PCB_SENSE].value, &s_adcChResult[ADC_PCB_SENSE].adcChStatus);

  
}

void Adc_Fast_Process(void)
{
  uint16_t value;
//  Cy_Adc_Channel_SoftwareTrigger(&PASS0_SAR1->CH[ADC1_HEAT_M]);
  Cy_Adc_Channel_GetResult(&PASS0_SAR1->CH[ADC1_HEAT_M], &value, &s_adcChResult[ADC_HEAT_M].adcChStatus);
  if(s_adcChResult[ADC_HEAT_M].adcChStatus.valid)
  {
    s_adcChResult[ADC_HEAT_M].value = value;
  }
}
void Adc_SetSampleTime_Dynamic(uint16_t newSampleTime)//an 2026.06.04    
{
  volatile stc_PASS_SAR_CH_t *base= &PASS0_SAR1->CH[ADC1_HEAT_M];
   // Cy_Adc_Channel_Disable(base);
    base->unSAMPLE_CTL.stcField.u12SAMPLE_TIME = newSampleTime;
//    Cy_Adc_Channel_Enable(base);
}
uint16_t Adc_Original_Val_Get(_em_adc_soft_trg typ)
{
  ...
  if(s_adcChResult[typ].adcChStatus.valid) 
    return s_adcChResult[typ].value;
