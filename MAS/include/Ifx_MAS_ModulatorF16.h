...
typedef struct Ifx_MAS_ModulatorF16
{
    ...
    bool p_enable;
    /** yxs**/
    bool p_forceDutyEnable;         // ?????????
    uint16 p_forceDutyU;            // U????(0-100%)
    uint16 p_forceDutyV;            // V????(0-100%)
    uint16 p_forceDutyW;            // W????(0-100%)
    bool p_forceSampleEnable;      // ???????
    uint16 p_forceSampleTick1;    // ???1 (tick)
    uint16 p_forceSampleTick2;    // ???2 (tick)
    /** yxs**/ 
    /**
 ...
 *  2. change to asymmetric switching if the current measurement window is not sufficient
 *  3. limit the compare values to prevent overflow
 *  4. calculate the current triggers
 *
...
/**
 *  \brief Get the modulator period
 *
 *
 *  \param [inout] self Reference to structure that contains instance data members
 *
 ...
 */
/**yxs**/
void Ifx_MAS_ModulatorF16_setForceDuty(Ifx_MAS_ModulatorF16* self, bool enable,
                                     uint16 dutyU, uint16 dutyV, uint16 dutyW);
void Ifx_MAS_ModulatorF16_setForceSample(Ifx_MAS_ModulatorF16* self, bool enable,
                                   uint16 sampleTick1, uint16 sampleTick2);
                                     /**yxs**/
...
/**
 *  \brief Set the modulator period
 *
 *  The period tick can be calculated based on the equation: peripheral frequency / switching frequency / 2 - 1.
 *
 *  \param [inout] self Reference to structure that contains instance data members
